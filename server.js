...
let utilities = require('./public/utilities.js'); 
...
class campaign {
	...
	async readSheet() {
		...
		if (readResult.status == '200') { //200 is success code
			...
			this.name = utilities.extractFromSSArr(this.spreadsheetArray, this.spreadsheetLayout.nameRange)[0];
			this.info = utilities.extractFromSSArr(this.spreadsheetArray, this.spreadsheetLayout.infoRange);
			this.coins = utilities.extractFromSSArr(this.spreadsheetArray, this.spreadsheetLayout.coinsRange).map(function (e) {
			...
			this.buckets = utilities.extractFromSSArr(this.spreadsheetArray, this.spreadsheetLayout.bucketsRange).filter(e => e != "");
			this.marketStartTime = utilities.ExcelDateToJSDate(utilities.extractFromSSArr(this.spreadsheetArray, this.spreadsheetLayout.marketStartRange)[0]).getTime();
			this.marketEndTime = utilities.ExcelDateToJSDate(utilities.extractFromSSArr(this.spreadsheetArray, this.spreadsheetLayout.marketEndRange)[0]).getTime();
		} else {
